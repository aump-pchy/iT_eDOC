        ...
        <div class="w-full lg:w-96 shrink-0">
          <div class="relative">
            <input 
              v-model="searchQuery"
              type="text" 
              placeholder="พิมพ์เลขที่, ปี พ.ศ., เรื่อง หรือ ผู้บันทึก..." 
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-all">
      <div class="bg-white rounded-2xl max-w-md w-full p-6 md:p-8 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
        ...
        <form @submit.prevent="submitUpdate">
          <div class="mb-4">
            <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">เลขที่บันทึกข้อความ ศส. (แก้ไขไม่ได้)</label>
            <input 
              type="text" 
              :value="editForm.memo_number" 
              disabled 
          ...
          <div class="mb-6">
            ...
            <div class="relative">
              <select 
                ...
                class="w-full px-3 py-2.5 border border-slate-300 rounded-xl font-bold text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none bg-no-repeat bg-right"
              >
