<template>
  <div class="min-h-screen bg-gray-50 py-8 px-6 md:pt-20">
    <div class="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-md border border-gray-200">
      ...
      <div class="mb-10">
        <h1 class="text-3xl font-semibold text-gray-800">จัดการข้อมูลผู้ใช้งาน</h1>
      </div>

      <div class="space-y-6">
        <div class="flex items-center gap-8">
          <label class="w-32 text-gray-700 font-medium">ชื่อ-สกุล</label>
        ...
        <div class="flex items-center gap-8">
          <label class="w-32 text-gray-700 font-medium">ตำแหน่ง</label>
          <input type="text" v-model="form.position" placeholder="กรอกตำแหน่ง"
            class="flex-1 border border-gray-400 rounded-xl p-2.5 bg-white text-black font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition shadow-sm" />
        </div>

        <div class="flex items-center gap-8">
          <label class="w-32 text-gray-700 font-medium">อีเมล</label>
          <input type="email" v-model="form.email" placeholder="example@email.com"
            class="flex-1 border border-gray-400 rounded-xl p-2.5 bg-white text-black font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition shadow-sm" />
        </div>

        <div class="flex items-center gap-8">
          <label class="w-32 text-gray-700 font-medium">รหัสผ่าน</label>
          <input type="password" v-model="form.password" placeholder="********"
            class="flex-1 border border-gray-400 rounded-xl p-2.5 bg-white text-black font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition shadow-sm" />
        </div>

        <div class="flex items-center gap-8">
          <label class="w-32 text-gray-700 font-medium">สิทธิการใช้งาน</label>
      ...
      <div class="mt-14">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 pb-2 border-b border-gray-100">
          <div class="flex items-center gap-4 w-full sm:w-auto flex-1">
            <h2 class="text-xl font-semibold text-gray-800 whitespace-nowrap">รายชื่อผู้ใช้งาน</h2>
...
const deleteUser = async (user) => {
  if (!confirm(`ต้องการลบข้อมูลของ "${user.name}" ใช่หรือไม่?`)) return
  try {
